...
class TurboHTTPTransport : public HTTPFileSystem {
public:
	...
	string GetName() const override {
		return "TurboHTTPTransport";
	}

	unique_ptr<HTTPResponse> HeadRequest(FileHandle &handle, string thfs_url, HTTPHeaders header_map) override {
		(void)thfs_url;
		auto &thfs_handle = handle.Cast<THFSFileHandle>();
		auto http_url = TurboHTTPFileSystem::GetFileUrl(thfs_handle.parsed_url);
		return HTTPFileSystem::HeadRequest(handle, http_url, header_map);
	}

	unique_ptr<HTTPResponse> GetRequest(FileHandle &handle, string thfs_url, HTTPHeaders header_map) override {
		(void)thfs_url;
		auto &thfs_handle = handle.Cast<THFSFileHandle>();
		auto http_url = TurboHTTPFileSystem::GetFileUrl(thfs_handle.parsed_url);
	...
	unique_ptr<HTTPResponse> GetRangeRequest(FileHandle &handle, string thfs_url, HTTPHeaders header_map,
...
protected:
	unique_ptr<HTTPFileHandle> CreateHandle(const OpenFileInfo &file, FileOpenFlags flags,
		...
		auto parsed_url = TurboHTTPFileSystem::THFSUrlParse(file.path, http_use_ssl);

		FileOpenerInfo info;
		info.file_path = file.path;
		auto http_util = HTTPFSUtil::GetHTTPUtil(opener);
		auto params = http_util->InitializeParameters(opener, info);
		TurboHTTPFileSystem::SetParams(params->Cast<HTTPFSParams>(), file.path, opener);

		return duckdb::make_uniq<THFSFileHandle>(*this, std::move(parsed_url), file, flags, std::move(params));
	}
};

TurboHTTPFileSystem::TurboHTTPFileSystem() : transport(make_uniq<TurboHTTPTransport>()) {
}
...
static string ListRequest(const ParsedTHFSUrl &url, HTTPFSParams &http_params, optional_ptr<HTTPState> state) {
	(void)state;
	HTTPHeaders header_map;
	std::stringstream response;
	auto list_url = TurboHTTPFileSystem::GetListUrl(url);
...
bool TurboHTTPFileSystem::CanHandleFile(const string &fpath) {
	return fpath.rfind("thfs://", 0) == 0;
}

string TurboHTTPFileSystem::GetName() const {
	return "TurboHTTPFileSystem";
}

string TurboHTTPFileSystem::PathSeparator(const string &path) {
	return "/";
}

bool TurboHTTPFileSystem::CanSeek() {
	return true;
}

bool TurboHTTPFileSystem::OnDiskFile(FileHandle &handle) {
	return false;
}

bool TurboHTTPFileSystem::IsPipe(const string &filename, optional_ptr<FileOpener> opener) {
	return false;
}

void TurboHTTPFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
	transport->Read(handle, buffer, nr_bytes, location);
}

int64_t TurboHTTPFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes) {
	return transport->Read(handle, buffer, nr_bytes);
}

void TurboHTTPFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
	transport->Write(handle, buffer, nr_bytes, location);
}

int64_t TurboHTTPFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes) {
	return transport->Write(handle, buffer, nr_bytes);
}

void TurboHTTPFileSystem::FileSync(FileHandle &handle) {
	transport->FileSync(handle);
}

int64_t TurboHTTPFileSystem::GetFileSize(FileHandle &handle) {
	return transport->GetFileSize(handle);
}

timestamp_t TurboHTTPFileSystem::GetLastModifiedTime(FileHandle &handle) {
	return transport->GetLastModifiedTime(handle);
}

string TurboHTTPFileSystem::GetVersionTag(FileHandle &handle) {
	return transport->GetVersionTag(handle);
}

bool TurboHTTPFileSystem::FileExists(const string &filename, optional_ptr<FileOpener> opener) {
	return transport->FileExists(filename, opener);
}

void TurboHTTPFileSystem::Seek(FileHandle &handle, idx_t location) {
	transport->Seek(handle, location);
}

idx_t TurboHTTPFileSystem::SeekPosition(FileHandle &handle) {
	return transport->SeekPosition(handle);
}

unique_ptr<FileHandle> TurboHTTPFileSystem::OpenFileExtended(const OpenFileInfo &file, FileOpenFlags flags,
                                                             optional_ptr<FileOpener> opener) {
	return transport->OpenFile(file.path, flags, opener);
}

bool TurboHTTPFileSystem::SupportsOpenFileExtended() const {
	return true;
}
